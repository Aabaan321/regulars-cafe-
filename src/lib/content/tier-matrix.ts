...
export const tierMatrix: readonly MatrixGroup[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      },
      {
        ...
        essential: NO,
        signature: NO,
        immersive: { kind: 'option', detail: '+₹5,000' },
        bespoke: YES,
      },
      {
        id: 'loyalty',
  ...
  },
  {
    ...
    rows: [
      {
        id: 'wa-booking',
        label: 'Booking confirmation on WhatsApp',
        labelAr: 'تأكيد الحجز عبر واتساب',
        note: 'Sent the moment a table is booked, with the time and how to find you.',
        essential: NO,
        signature: YES,
        immersive: YES,
        bespoke: YES,
      ...
      {
        id: 'wa-reminder',
        label: 'Reminder before the booking',
        labelAr: 'تذكير قبل موعد الحجز',
        essential: NO,
        signature: { kind: 'yes', detail: 'Day before' },
        immersive: { kind: 'yes', detail: 'Day before + 2h' },
        bespoke: { kind: 'custom' },
      },
      {
        id: 'wa-thanks',
        label: 'Thank-you message after the visit',
        labelAr: 'رسالة شكر بعد الزيارة',
        note: '“Hope you enjoyed it — see you next time”, with a voucher attached.',
        essential: NO,
        signature: YES,
        immersive: YES,
        bespoke: YES,
      ...
      {
        id: 'wa-voucher',
        label: 'Discount voucher with a tracked code',
        labelAr: 'قسيمة خصم برمز قابل للتتبع',
        note: 'You see how many came back and what it cost you.',
        essential: NO,
        signature: { kind: 'yes', detail: 'Fixed voucher' },
        immersive: { kind: 'yes', detail: 'Rules-based' },
        bespoke: { kind: 'custom' },
      },
      {
        id: 'wa-journey',
        label: 'Full automated journey',
        labelAr: 'رحلة مؤتمتة كاملة',
        note: 'Win-back for guests who stop coming, birthdays, order-ready pings, campaigns.',
        essential: NO,
        signature: NO,
        immersive: YES,
        bespoke: YES,
      ...
      {
        id: 'wa-inbox',
        label: 'Two-way inbox for your team',
        labelAr: 'صندوق وارد ثنائي الاتجاه لفريقك',
        note: 'Guests reply and a human picks it up, rather than shouting into a void.',
        essential: NO,
        signature: { kind: 'option', detail: 'Add-on' },
        immersive: YES,
        bespoke: YES,
      },
    ],
  ...
  },
  {
    ...
    rows: [
      ...
      },
      {
        ...
        note: 'Basic gets you found. Premium is what the brands above you are running.',
        essential: { kind: 'yes', detail: 'Basic' },
        signature: { kind: 'yes', detail: 'Advanced' },
        immersive: { kind: 'yes', detail: 'Premium' },
        bespoke: { kind: 'custom', detail: 'Premium +' },
      },
      {
        id: 'seo-detail',
        label: 'What that actually covers',
        labelAr: 'ما الذي يشمله ذلك فعلياً',
        essential: {
          kind: 'yes',
          detail: 'Titles, meta, sitemap, schema.org, Google Business Profile',
        },
        signature: {
          kind: 'yes',
          detail:
            'The above + local keyword research, bilingual hreflang, internal linking, 3 months of Search Console reporting',
        },
        immersive: {
          kind: 'yes',
          detail:
            'The above + competitor gap analysis, review and FAQ schema, monthly content plan, Core Web Vitals held to target',
        },
        bespoke: { kind: 'custom' },
      },
      {
  ...
  },
  {
    ...
    rows: [
      ...
      },
      {
        ...
        essential: { kind: 'option', detail: 'From ₹2,000/mo' },
        signature: { kind: 'option', detail: 'From ₹4,000/mo' },
        immersive: { kind: 'option', detail: 'From ₹7,500/mo' },
        bespoke: { kind: 'custom' },
      },
      {
        id: 'ownership',
...
export const tierCaveats: Record<string, { en: string; ar: string }> = {
  essential: {
    en: 'No bookings, no ordering, no WhatsApp automation, and no support window after we hand it over. SEO is the basics only. It is a brochure — a fast, findable, well-written one.',
    ar: 'بلا حجوزات ولا طلبات ولا عربية ولا فترة دعم بعد التسليم. إنه موقع تعريفي — سريع وواضح ومكتوب جيداً.',
  },
  signature: {
    en: 'No film, no AI, no order-at-table, and WhatsApp stops at confirmations and thank-yous. Everything that makes the café *run* on the site is here; the cinema is not.',
